...
const generatedArray = (ySize: number, xSize: number) => {
    return Array.from(Array(ySize)).map(item => Array.from(Array(xSize)).map(item => Math.random() > 0.5 ? 1 : 0))
}
...
export const GameField: FC<Props> = ({xSize, ySize}) => {
    ...
    return ( <div>
            ...
            onClick={handleClick}
            ></canvas>
            <button onClick={() => setGameState({running: !gameState.running})}>{gameState.running ? 'Stop' : 'Start'}</button>
        </div>
    )
}